...
Canvas.prototype.touchMove = function(event) {
  ...
  this.target = new Vector(
    event.touches[0].pageX * this.dpr,
    event.touches[0].pageY * this.dpr
  )
}
...
Canvas.prototype.drawLines = function(point, color) {
  ...
  while (++index < length) {
    ...
    this.ctx.strokeStyle =
      'rgba(' + color + ', ' + point.closest[index].opacity + ')'
    this.ctx.beginPath()
...
function Particle(options) {
  ...
  if (this.options.color) {
    ...
    var colorIndex = -1
    while (++colorIndex < 3) {
      color[colorIndex] = Math.round(
        parseInt(color[colorIndex], 10) + Math.random() * 100 - 50
      )
